...
const GuestInfoForm = ({ hotelId, pricePerNight }: Props) => {
  ...
  const checkIn = watch("checkIn") as Date;
  const checkOut = watch("checkOut") as Date;
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          ...
          <DatePicker
            required
            selected={checkIn}
            onChange={(date: Date | null) => date && setValue("checkIn", date)}
            selectsStart
            startDate={checkIn}
            endDate={checkOut}
            minDate={minDate}
          ...
          <DatePicker
            required
            selected={checkOut}
            onChange={(date: Date | null) => date && setValue("checkOut", date)}
            selectsEnd
            startDate={checkIn}
            endDate={checkOut}
            minDate={minDate}
